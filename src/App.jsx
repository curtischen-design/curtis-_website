    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="pt-32 px-8 md:px-24 max-w-5xl mx-auto pb-60 text-white font-sans">
      ...
      <div className="space-y-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 text-white">
          ...
          <div className="space-y-4">
            <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold ml-1 flex items-center gap-2 text-white text-white"><Calendar size={12}/> 2. 標記時間</label>
        ...
        <div className="space-y-4">
          ...
          <input className="w-full bg-transparent border-b border-white/10 py-2 text-lg outline-none focus:border-white text-white text-white" value={pathInput} onChange={e => setPathInput(e.target.value)} placeholder="Root / Subpath" />
        </div>
      ...
      <div className="mt-40 space-y-12 border-t border-white/10 pt-40">
        <div className="p-10 bg-white/5 rounded-[60px] border border-white/5 space-y-10">
          <div className="flex justify-between items-center">
            <h3 className="text-[10px] uppercase tracking-[0.4em] text-white/30 flex items-center gap-3 text-white"><Type size={14}/> 全站品牌與標語設定</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 text-white">
             ...
             <div className="space-y-4">
                <label className="text-[9px] uppercase tracking-widest text-white/20 ml-2 font-bold">首頁分段標題 (直式)</label>
          ...
          <div className="space-y-4">
            <label className="text-[9px] uppercase tracking-widest text-white/20 ml-2 font-bold">首頁描述大標題</label>
            <input className="w-full bg-black/30 border border-white/5 rounded-full px-6 py-3 text-sm outline-none focus:border-[#368C84] text-white" value={tempSiteSettings.heroSubtitle || 'Digital Curations by Curtis Chen'} onChange={e => setTempSiteSettings({...tempSiteSettings, heroSubtitle: e.target.value})} />
          </div>

          <div className="space-y-4">
            <label className="text-[9px] uppercase tracking-widest text-white/20 ml-2 font-bold">首頁描述文字內容</label>
            <textarea rows={3} className="w-full bg-black/30 border border-white/5 rounded-[30px] p-6 text-sm outline-none focus:border-[#368C84] text-white" value={tempSiteSettings.heroDescription || '致力於收藏生活中的美學動態...'} onChange={e => setTempSiteSettings({...tempSiteSettings, heroDescription: e.target.value})} />
          </div>
        </div>
      </div>
      ...
      <div className="mt-20 space-y-12">
        <div className="p-10 bg-white/5 rounded-[60px] border border-white/5 space-y-10">
          <h3 className="text-[10px] uppercase tracking-[0.4em] text-white/30 flex items-center gap-3 text-white"><Star size={14}/> 選單策展設定 (精選與名言)</h3>
...
export default function App() {
  ...
  const [siteSettings, setSiteSettings] = useState({
     logoName: 'Curtis Chen',
     title1: '博物館',
     title2: '部落格',
     heroSubtitle: 'Digital Curations by Curtis Chen',
     heroDescription: '致力於收藏生活中的美學動態，將瑣碎的感知轉化為永恆的數位檔案。'
  });
  ...
  return (
    <div className="bg-[#368C84] text-white min-h-screen font-sans selection:bg-white selection:text-[#368C84] text-white overflow-x-hidden">
      ...
      <motion.nav animate={{ y: showHeader ? 0 : -100 }} className="fixed top-0 left-0 w-full p-8 flex justify-between items-center z-[110] mix-blend-difference font-black tracking-tighter uppercase italic text-white text-white">
        <div onClick={() => { setView({type:'home'}); setActiveMenu('All'); setActiveSubPath('All'); window.scrollTo(0,0); }} className="text-xl cursor-pointer hover:opacity-50 transition-all text-white">
          ...
          <motion.main key={activeMenu} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pt-40 px-8 md:px-24 pb-40 text-white font-sans text-white">
            <div className="flex flex-row items-start gap-12 mb-32 text-white text-white">
              <motion.div initial={{ x: -40, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={transition} className="flex gap-4 text-white">
                ...
                <div className="text-7xl md:text-[9.5vw] font-black leading-none uppercase tracking-tighter font-serif vertical-text pt-32 italic text-white text-white">
              ...
              <div className="flex-1 space-y-6 pt-16 hidden md:block text-white/60 text-white">
                <p className="text-xs uppercase tracking-[0.6em] text-white/40 border-b border-white/10 pb-6 font-black tracking-[0.4em] text-white">
                   {activeMenu === 'All' ? (siteSettings.heroSubtitle || 'Digital Curations') : `Archives / ${activeMenu}`}
                </p>
            ...
            <div className="grid grid-cols-1 text-white">
              {activeMenu !== 'All' ? (
                Object.entries(groupedArticles).map(([group, arts]) => (
                  <div key={group} className="mb-24 text-white">
                     ...
                     <div className="space-y-0 text-white text-white">
                        {arts.map((art) => (
                          <motion.div key={art.id} onClick={() => setView({ type: 'article', id: art.id })} className="group py-12 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center cursor-pointer hover:bg-white/[0.02] px-8 -mx-8 relative">
                             <div className="flex-1 text-white">
                ...
                processedArticles.map((art) => (
                  <motion.div key={art.id} onClick={() => setView({ type: 'article', id: art.id })} className="group py-16 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center cursor-pointer hover:bg-white/[0.02] transition-all px-8 -mx-8 relative text-white">
                    <div className="flex-1 space-y-4">
                      ...
                      <h3 className="text-4xl md:text-7xl font-serif leading-tight group-hover:font-black group-hover:italic text-white text-white">{art.title}</h3>
          ...
          <motion.article key="article" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={transition} className="pt-40 px-8 md:px-24 max-w-6xl mx-auto pb-60 text-white font-sans text-white text-white">
            ...
            {articles.find(a => a.id === view.id) && (
              <>
                ...
                <div className="flex flex-col items-center gap-8 mb-40 border-y border-white/5 py-12 text-white text-white">
                ...
                <div className="max-w-3xl mx-auto mb-60 space-y-16 text-white text-white text-white">
                    ...
                    {comments.map(c => (
                      <div key={c.id} className="group flex gap-6 items-start text-white text-white text-white">
                        <div className="w-12 h-12 rounded-full bg-white/5 border border-white/10 flex-shrink-0 overflow-hidden shadow-2xl text-white text-white text-white">
                          {c.userPhoto ? <img src={c.userPhoto} alt="" className="w-full h-full object-cover text-white" /> : <div className="w-full h-full flex items-center justify-center text-white/10 italic font-black font-serif uppercase text-white">?</div>}
                        </div>
                ...
                <div className="pt-32 border-t border-white/10 flex flex-col items-center text-white text-white text-white">
                    <p className="text-[10px] uppercase tracking-[0.6em] text-white/20 mb-12 italic font-black text-white/40 text-white">Next Exhibition</p>
                    <div className="w-full flex flex-row items-center justify-between gap-4 md:gap-12 px-2 text-white text-white text-white">
                        <div onClick={() => { setView({type:'home'}); window.scrollTo(0,0); }} className="flex items-center gap-6 group cursor-pointer text-white flex-shrink-0 text-white text-white text-white">
                            <div className="p-4 md:p-6 rounded-full border border-white/20 group-hover:bg-white group-hover:text-black transition-all duration-700 shadow-2xl flex items-center justify-center text-white text-white text-white">
                                <ArrowLeft size={20} color="white" className="group-hover:stroke-black text-white text-white text-white text-white text-white" />
                            </div>
                        ...
                        <div onClick={() => { setView({type:'home'}); setActiveMenu('All'); window.scrollTo(0,0); }} className="flex items-center gap-6 group cursor-pointer text-white flex-shrink-0 text-white text-white text-white">
                            <span className="text-[9px] md:text-[11px] uppercase tracking-[0.4em] text-white/30 group-hover:text-white whitespace-nowrap font-black hidden lg:inline text-white text-white text-white">Return Home</span>
