...
const renderMarkdown = (text) => {
  ...
  // 1. Obsidian Callout 渲染 (穩定捕捉多行內容，處理 > 符號)
  ...
  processed = processed.replace(calloutRegex, (match, type, title, content) => {
    ...
    if (type.toLowerCase() === 'quote') {
      return `<div class="my-12 p-8 bg-white/5 border-l-4 border-white/40 rounded-r-3xl italic font-serif text-white/90 shadow-inner relative">
                <div class="text-xl md:text-2xl leading-relaxed">“${innerContent}”</div>
              </div>`;
    } else {
      return `<div class="my-12 p-8 bg-[#368C84]/20 border-l-4 border-[#368C84] rounded-r-3xl shadow-xl">
                <div class="flex items-center gap-3 mb-4 opacity-50 font-black text-white text-white">
                  ...
                  <span class="uppercase tracking-[0.4em] text-[10px] text-white">System Information</span>
                </div>
...
const CMSDashboard = ({ user, setView, editData, showMessage, menuList, featuredMenus, menuQuotes }) => {
  ...
  const toggleFMenu = async (name) => {
    const currentItems = featuredMenus || [];
    const updated = currentItems.includes(name) ? currentItems.filter(m => m !== name) : [...currentItems, name];
    await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'featuredMenuSettings'), { items: updated });
    showMessage(`${name} 狀態已更新`);
  };
  ...
  const handlePublish = async (isDraft = false) => {
    ...
    try {
      ...
      showMessage('發布完成');
      setView({ type: 'home' });
    } catch (err) { showMessage('發布失敗'); } finally { setLoading(false); }
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="pt-32 px-8 md:px-24 max-w-4xl mx-auto pb-40 text-white font-sans">
      ...
      <div className="space-y-16">
        ...
        <div className="space-y-4">
          <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold ml-1">3. 標題與置頂 (Star)</label>
        ...
        <div className="space-y-4 text-white">
          ...
          <input className="w-full bg-transparent border-b border-white/10 py-2 text-lg outline-none focus:border-white text-white" value={pathInput} onChange={e => setPathInput(e.target.value)} placeholder="Path / Subpath" />
        </div>
      ...
      <div className="mt-40 space-y-12 border-t border-white/10 pt-40">
        <div className="p-10 bg-white/5 rounded-[60px] border border-white/5 space-y-10">
          <h3 className="text-[10px] uppercase tracking-[0.4em] text-white/30 flex items-center gap-3"><Star size={14}/> 選單策展設定 (常駐與名言)</h3>
...
export default function App() {
  ...
  useEffect(() => { 
    onAuthStateChanged(auth, (u) => {
      setUser(u);
      if (!u) signInAnonymously(auth); // 確保未登入者也能互動
    });
  }, []);
  ...
  return (
    <div className="bg-[#368C84] text-white min-h-screen font-sans selection:bg-white selection:text-[#368C84] text-white overflow-x-hidden">
      ...
      <AnimatePresence mode="wait">
        {/* 首頁與分類展廳 (類首頁) */}
        {view.type === 'home' && (
          <motion.main key={activeMenu} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pt-40 px-8 md:px-24 pb-40 text-white font-sans">
            <div className="flex flex-row items-start gap-12 mb-32">
              ...
              <div className="flex-1 space-y-6 pt-16 hidden md:block text-white/60">
                <p className="text-xs uppercase tracking-[0.6em] text-white/40 border-b border-white/10 pb-6 font-black text-white text-white">
            ...
            <div className="grid grid-cols-1">
              {activeMenu !== 'All' ? (
                ...
                processedArticles.map((art) => (
                  <motion.div key={art.id} onClick={() => setView({ type: 'article', id: art.id })} className="group py-16 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center cursor-pointer hover:bg-white/[0.02] transition-all px-8 -mx-8 relative text-white text-white text-white text-white">
        ...
        {view.type === 'article' && (
          <motion.article key="article" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={transition} className="pt-40 px-8 md:px-24 max-w-6xl mx-auto pb-60 text-white font-sans text-white text-white text-white">
            <div className="flex justify-between items-center mb-16 border-b border-white/5 pb-8 text-white text-white text-white text-white">
              <div className="flex items-center gap-4 text-[10px] uppercase tracking-widest text-white/20 font-serif italic text-white text-white text-white text-white text-white">
            ...
            {articles.find(a => a.id === view.id) && (
              <>
                <h1 className="text-5xl md:text-[7.5vw] font-black font-serif mb-24 leading-[1.05] tracking-tighter italic text-white text-white text-white text-white text-white">{articles.find(a => a.id === view.id).title}</h1>
                ...
                {/* 反應區 */}
                <div className="flex flex-col items-center gap-8 mb-40 border-y border-white/5 py-12 text-white text-white text-white text-white text-white text-white">
                  <div className="flex gap-4 flex-wrap justify-center text-white text-white text-white text-white text-white">
                ...
                {/* 留言區 */}
                <div className="max-w-3xl mx-auto mb-60 space-y-16 text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white">
                  <h3 className="text-2xl font-serif italic flex items-center gap-4 text-white text-white text-white text-white text-white">檔案評論區 <span className="text-xs font-mono opacity-20 tracking-tighter text-white">({comments.length})</span></h3>
                    ...
                    {comments.map(c => (
                      <div key={c.id} className="group flex gap-6 items-start text-white text-white text-white">
                        <div className="w-12 h-12 rounded-full bg-white/5 border border-white/10 flex-shrink-0 overflow-hidden shadow-2xl text-white text-white text-white">
                          {c.userPhoto ? <img src={c.userPhoto} alt="" className="w-full h-full object-cover text-white text-white" /> : <div className="w-full h-full flex items-center justify-center text-white/10 italic font-black font-serif uppercase text-white">?</div>}
                        </div>
                        <div className="flex-1 space-y-2 text-white text-white text-white">
                          <div className="flex items-center justify-between text-white text-white text-white text-white">
                            ...
                            <div className="flex gap-4 opacity-0 group-hover:opacity-100 transition-opacity text-white text-white">
                              {(c.userId === user?.uid || isAdmin) && <button onClick={() => deleteComment(c.id)} className="text-white/20 hover:text-red-400 transition-all cursor-pointer text-white text-white text-white"><Trash2 size={14} color="white"/></button>}
                            </div>
                          </div>
                          <p className="text-white/60 font-serif text-lg leading-relaxed text-white text-white text-white text-white text-white text-white">${c.text}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  <div className="relative pt-12 text-white text-white text-white text-white text-white text-white text-white">
                    ...
                    {user ? (
                      <button onClick={postComment} className="absolute bottom-8 right-10 bg-white text-[#368C84] px-10 py-4 rounded-full font-black text-xs flex items-center gap-4 cursor-pointer shadow-2xl hover:scale-105 transition-all text-[#368C84] text-[#368C84] text-[#368C84]">Submit <Send size={18} color="#368C84"/></button>
                    ) : (
                      <button onClick={() => signInWithPopup(auth, provider)} className="absolute bottom-8 right-10 bg-white/10 text-white px-10 py-4 rounded-full font-black text-xs cursor-pointer hover:bg-white/20 transition-all text-white text-white text-white text-white text-white text-white">Sign In to Post</button>
                    )}
                  </div>
                </div>

                {/* 一體化導覽列 */}
                <div className="pt-32 border-t border-white/10 flex flex-col items-center text-white text-white text-white text-white">
                    ...
                    <div className="w-full flex flex-row items-center justify-between gap-4 md:gap-12 px-2 text-white text-white text-white text-white">
                        <div onClick={() => { setView({type:'home'}); window.scrollTo(0,0); }} className="flex items-center gap-6 group cursor-pointer text-white flex-shrink-0 text-white text-white text-white text-white text-white">
                        ...
                        {articles.filter(a => a.id !== view.id)[0] && (
                           <motion.div whileHover={{ scale: 1.05 }} onClick={() => { setView({type:'article', id: articles.filter(a => a.id !== view.id)[0].id}); window.scrollTo(0,0); }} className="flex flex-col items-center group cursor-pointer max-w-[50%] md:max-w-xl text-center text-white overflow-hidden text-white text-white text-white">
                              <h4 className="text-xl md:text-4xl font-serif text-white transition-all leading-tight tracking-tighter group-hover:italic group-hover:font-black truncate w-full text-white text-white text-white text-white text-white text-white text-white text-white text-white">
      ...
      <footer className="p-12 md:p-24 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 text-[8px] text-white/10 uppercase tracking-[0.6em] font-light italic text-white/10 text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white">
        <SocialIconsList className="text-white text-white text-white text-white text-white" />
        <InteractiveLink className="font-black text-white/20 text-white text-white text-white text-white text-white text-white" onClick={() => setView({type:'home'})}>© 2026 CURTIS CHEN — MUSEUM BLOG ARCHIVE</InteractiveLink>
