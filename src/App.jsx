...
const renderMarkdown = (text) => {
  ...
  // 1. Obsidian Callout 渲染 (對標圖片樣式，修復正則與 > 符號處理)
  ...
  processed = processed.replace(calloutRegex, (match, type, title, content) => {
    ...
    if (type.toLowerCase() === 'quote') {
      ...
      return `<div class="my-10 p-8 bg-[#368C84]/20 border-l-4 border-[#368C84] rounded-r-3xl shadow-xl text-white">
                <div class="flex items-center gap-3 mb-4 opacity-50 font-black text-white"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg><span class="uppercase tracking-[0.4em] text-[10px] text-white">Archive Notice</span></div>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="pt-32 px-8 md:px-24 max-w-5xl mx-auto pb-60 text-white font-sans">
      <div className="flex justify-between items-end mb-16 border-b border-white/10 pb-8 text-white">
        <h2 className="text-4xl font-serif italic text-white">{editData ? '編輯館藏.' : '新增檔案.'}</h2>
      ...
      <div className="space-y-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 text-white">
          <div className="space-y-4">
            <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold ml-1">1. 檔案分類</label>
      ...
      <div className="mt-40 space-y-12 border-t border-white/10 pt-40 text-white">
        <div className="p-10 bg-white/5 rounded-[60px] border border-white/5 space-y-10">
          <div className="flex justify-between items-center text-white">
            <h3 className="text-[10px] uppercase tracking-[0.4em] text-white/30 flex items-center gap-3 text-white"><Type size={14}/> 全站文案與標語自定義</h3>
          ...
          <div className="space-y-4 text-white">
            <label className="text-[9px] uppercase tracking-widest text-white/20 ml-2 font-bold text-white text-white">首頁描述大標題與正文</label>
        ...
        <div className="p-10 bg-white/5 rounded-[60px] border border-white/5 space-y-10 text-white">
          <h3 className="text-[10px] uppercase tracking-[0.4em] text-white/30 flex items-center gap-3 text-white text-white"><Star size={14}/> 選單策展設定 (精選與名言)</h3>
...
export default function App() {
  ...
  const [siteSettings, setSiteSettings] = useState({
     ...
     heroSubtitle: 'Digital Curations by Curtis Chen', heroDescription: '致力於收藏生活中的美學動態，將瑣碎的感知轉化為永恆的數位檔案。'
  });
  ...
  // 初始化與數據同步 (對標 Step 3: Realtime Listeners)
  useEffect(() => { 
  ...
  return (
    <div className="bg-[#368C84] text-white min-h-screen font-sans selection:bg-white selection:text-[#368C84] text-white overflow-x-hidden">
      ...
      <motion.nav animate={{ y: showHeader ? 0 : -100 }} className="fixed top-0 left-0 w-full p-8 flex justify-between items-center z-[110] mix-blend-difference font-black tracking-tighter uppercase italic text-white text-white">
        <div onClick={() => { setView({type:'home'}); setActiveMenu('All'); setActiveSubPath('All'); setSearchQuery(''); window.scrollTo(0,0); }} className="text-xl cursor-pointer hover:opacity-50 transition-all">
           {siteSettings.logoName || 'Curtis Chen'}
        </div>
        <div className="flex items-center gap-6">
          <button onClick={() => setIsSearchOpen(!isSearchOpen)} className="hover:opacity-50 cursor-pointer transition-all"><Search size={24} color="white"/></button>
          <motion.button 
             key={menuPulseKey}
          ...
          <motion.main key={activeMenu} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pt-40 px-8 md:px-24 pb-40 text-white font-sans text-white text-white text-white">
            <div className="flex flex-row items-start gap-12 mb-32">
              <motion.div initial={{ x: -40, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={transition} className="flex gap-4">
                ...
                <div className="text-7xl md:text-[9.5vw] font-black leading-none uppercase tracking-tighter font-serif vertical-text pt-32 italic text-white text-white text-white text-white">
                   {activeMenu === 'All' ? (siteSettings.title2 || '部落格') : '展覽室'}
                </div>
              </motion.div>
              <div className="flex-1 space-y-6 pt-16 hidden md:block text-white/60">
                <p className="text-xs uppercase tracking-[0.6em] text-white/40 border-b border-white/10 pb-6 font-black tracking-[0.4em] text-white text-white text-white text-white text-white">
            ...
            <div className="space-y-8 mb-24 sticky top-0 bg-[#368C84]/90 backdrop-blur-lg z-[100] py-6 border-b border-white/5 text-white">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 text-white text-white text-white">
                <div className="flex flex-col text-white text-white text-white">
                  <div className="flex items-center gap-4 overflow-x-auto scrollbar-hide font-black text-white text-white text-white text-white text-white text-white text-white text-white">
                ...
                <div className="flex items-center gap-4 text-[9px] uppercase tracking-[0.2em] font-black text-white/40 self-end md:self-auto text-white">
                   <SortAsc size={12} color="white"/><span className="text-white text-white">排序</span>
                ...
                processedArticles.map((art) => (
                  <motion.div key={art.id} onClick={() => setView({ type: 'article', id: art.id })} className="group py-16 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center cursor-pointer hover:bg-white/[0.02] transition-all px-8 -mx-8 relative text-white text-white">
                    <div className="flex-1 space-y-4 text-white">
                      <div className="flex items-center gap-3 flex-wrap text-[10px] uppercase tracking-widest text-white/40 italic font-bold text-white text-white">
                        ...
                        {art.status === 'draft' && <span className="bg-yellow-500/20 text-yellow-500 px-2 rounded-full text-[8px] tracking-widest border border-yellow-500/20 ml-2 text-white text-white">DRAFT</span>}
                      </div>
            ...
            {articles.find(a => a.id === view.id) && (
              <>
                ...
                <div className="max-w-3xl mx-auto mb-60 space-y-16 text-white text-white">
                  <h3 className="text-2xl font-serif italic flex items-center gap-4 text-white text-white">檔案評論區 <span className="text-xs font-mono opacity-20 tracking-tighter text-white">({comments.length})</span></h3>
                    ...
                    {comments.map(c => (
                      <div key={c.id} className="group flex gap-6 items-start text-white">
                        <div className="w-12 h-12 rounded-full bg-white/5 border border-white/10 flex-shrink-0 overflow-hidden shadow-2xl text-white text-white">
                          {c.userPhoto ? <img src={c.userPhoto} alt="" className="w-full h-full object-cover text-white" /> : <div className="w-full h-full flex items-center justify-center text-white/10 italic font-black font-serif uppercase text-white text-white">?</div>}
                        </div>
                        <div className="flex-1 space-y-2 text-white text-white text-white">
                          <div className="flex items-center justify-between text-white text-white text-white">
                            ...
                            <div className="flex gap-4 opacity-0 group-hover:opacity-100 transition-opacity text-white text-white">
                          ...
                          <p className="text-white/60 font-serif text-lg leading-relaxed text-white text-white">${c.text}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  <div className="relative pt-12 text-white text-white text-white">
                    ...
                    <div className="flex justify-end mt-4 text-white text-white text-white">
                ...
                <div className="pt-32 border-t border-white/10 flex flex-col items-center text-white text-white text-white">
                    ...
                    <div className="w-full flex flex-row items-center justify-between gap-4 md:gap-12 px-2 text-white text-white text-white text-white text-white">
                        ...
                        {articles.filter(a => a.id !== view.id)[0] && (
                           <motion.div whileHover={{ scale: 1.05 }} onClick={() => { setView({type:'article', id: articles.filter(a => a.id !== view.id)[0].id}); window.scrollTo(0,0); }} className="flex flex-col items-center group cursor-pointer max-w-[50%] md:max-w-xl text-center text-white overflow-hidden text-white text-white text-white text-white text-white">
                        ...
                        <div onClick={() => { setView({type:'home'}); setActiveMenu('All'); window.scrollTo(0,0); }} className="flex items-center gap-6 group cursor-pointer text-white flex-shrink-0 text-white text-white text-white">
                            <span className="text-[9px] md:text-[11px] uppercase tracking-[0.4em] text-white/30 group-hover:text-white whitespace-nowrap font-black hidden lg:inline text-white">Return Home</span>
      ...
      <footer className="p-12 md:p-24 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 text-[8px] text-white/10 uppercase tracking-[0.6em] font-light italic text-white/10 text-white text-white text-white text-white">
        <InteractiveLink className="font-black text-white/20 text-white" onClick={() => setView({type:'home'})}>© 2026 CURTIS CHEN — MUSEUM BLOG ARCHIVE</InteractiveLink>
