...
import { 
  ...
  Instagram, Music, Headphones, Link as LinkIcon, Calendar, Settings2, XCircle, User as UserIcon, FileText, Globe, Star, SortAsc, Save
} from 'lucide-react';
...
const renderMarkdown = (text) => {
  ...
  processed = processed.replace(calloutRegex, (match, type, title, content) => {
    ...
    if (type.toLowerCase() === 'quote') {
      return `<div class="my-10 p-8 bg-white/5 border-l-4 border-white/40 rounded-r-3xl italic font-serif text-white/90 shadow-inner relative text-white">
                <div class="text-xl md:text-2xl leading-relaxed">“${innerContent}”</div>
              </div>`;
    } else {
      return `<div class="my-10 p-8 bg-[#368C84]/20 border-l-4 border-[#368C84] rounded-r-3xl shadow-xl text-white">
                <div class="flex items-center gap-3 mb-4 opacity-50 font-black text-white">
                  ...
                  <span class="uppercase tracking-[0.4em] text-[10px] text-white">Archive Notice</span>
                </div>
...
const CMSDashboard = ({ user, setView, editData, showMessage, menuList, articles, featuredMenus, menuQuotes }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="pt-32 px-8 md:px-24 max-w-4xl mx-auto pb-60 text-white font-sans">
      <div className="flex justify-between items-end mb-16 border-b border-white/10 pb-8 text-white text-white text-white">
        <h2 className="text-4xl font-serif italic text-white text-white">{editData ? '編輯館藏.' : '新增檔案.'}</h2>
        <InteractiveLink onClick={() => setView({ type: 'home' })} className="text-white text-white">Back</InteractiveLink>
      </div>

      <div className="space-y-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 text-white text-white text-white">
          <div className="space-y-4">
            <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold ml-1 text-white text-white">1. 選單分類</label>
            ...
            <div className="flex flex-wrap gap-2 mt-4 text-white text-white text-white">
              {menuList.map(m => (
          ...
          <div className="space-y-4 text-white">
            <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold ml-1 flex items-center gap-2 text-white text-white text-white"><Calendar size={12}/> 2. 標記時間</label>
            <input type="datetime-local" className="w-full bg-white/5 border border-white/10 rounded-full px-6 py-3 text-sm outline-none focus:border-[#368C84] transition-all text-white text-white text-white" value={pubDate} onChange={e => setPubDate(e.target.value)} />
          </div>
        </div>

        <div className="space-y-4">
          <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold ml-1 text-white text-white">3. 標題與訂選 (Star)</label>
          <div className="flex gap-4 items-center text-white">
            <input required className="flex-1 bg-transparent border-b border-white/10 py-4 text-3xl font-serif outline-none focus:border-[#368C84] text-white text-white text-white" value={title} onChange={e => setTitle(e.target.value)} placeholder="Title..." />
        ...
        <div className="space-y-4 text-white text-white text-white text-white text-white">
          <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold ml-1 text-white text-white">4. 子路徑 (如: 大一 / 生活)</label>
          <input className="w-full bg-transparent border-b border-white/10 py-2 text-lg outline-none focus:border-white text-white text-white text-white text-white text-white text-white text-white text-white" value={pathInput} onChange={e => setPathInput(e.target.value)} placeholder="Root / Path / Subpath" />
        </div>
        ...
        <div className="space-y-4 text-white text-white">
          ...
          <textarea required rows={12} className="w-full bg-black/10 border border-white/5 p-8 rounded-[40px] text-white/80 font-mono text-sm leading-relaxed outline-none focus:border-[#368C84]/50 transition-all text-white text-white text-white text-white" value={content} onChange={e => setContent(e.target.value)} />
        </div>

        <div className="flex flex-col md:flex-row gap-6">
          <button disabled={loading} onClick={() => handlePublish(false)} className="flex-1 bg-white text-[#368C84] py-8 rounded-full font-bold uppercase tracking-[0.5em] hover:scale-[0.98] transition-all shadow-2xl flex items-center justify-center gap-3 text-[#368C84] text-[#368C84]">
            <Globe size={18}/> 正式發布
          </button>
      ...
      <div className="mt-40 space-y-12 border-t border-white/10 pt-40">
        <h3 className="text-[10px] uppercase tracking-[0.6em] text-white/30 flex items-center gap-3 text-white"><FileText size={14}/> 草稿檔案庫 (按分類層級排列)</h3>
              ...
              {Object.entries(subGroups).map(([sub, arts]) => (
                <div key={sub} className="ml-6 space-y-2">
                  ...
                  <div className="flex flex-col">
                    {arts.map(art => (
                      <button key={art.id} onClick={() => { setView({type:'cms'}); setEditingArticle(art); window.scrollTo(0,0); }} className="text-left py-2 border-b border-white/5 hover:text-[#368C84] transition-colors group flex items-center justify-between text-white">
      ...
      <div className="mt-40 space-y-12 border-t border-white/10 pt-40 text-white">
        <div className="p-10 bg-white/5 rounded-[60px] border border-white/5 space-y-10">
          <h3 className="text-[10px] uppercase tracking-[0.4em] text-white/30 flex items-center gap-3 text-white"><Star size={14}/> 精選與名言同步設定</h3>
          <div className="grid grid-cols-1 gap-8 text-white text-white">
            {menuList.map(name => (
              <div key={name} className="flex flex-col md:flex-row md:items-center gap-6 bg-white/[0.02] p-8 rounded-[40px] border border-white/5 text-white">
                ...
                >
                  {featuredMenus.includes(name) ? '已設精選' : '設為精選'}
                </button>
...
export default function App() {
  ...
  const processedArticles = useMemo(() => {
    ...
    if (sortMode === 'time') list.sort((a, b) => b.publishDate.toDate() - a.publishDate.toDate());
      ...
      list.sort((a, b) => (b.reactionData?.[type] || 0) - (a.reactionData?.[type] || 0));
    }
  ...
  const postComment = async () => {
    ...
    await addDoc(coll, { text: newComment, userId: user.uid, userName: user.displayName || '訪客員', userPhoto: user.photoURL, createdAt: Timestamp.now() });
    setNewComment("");
  };
  ...
  return (
    <div className="bg-[#368C84] text-white min-h-screen font-sans selection:bg-white selection:text-[#368C84] text-white overflow-x-hidden">
      ...
      <div className="fixed top-0 left-0 right-0 h-10 group z-[150] cursor-default text-white text-white text-white">
        ...
        <div className="opacity-0 group-hover:opacity-100 transition-all duration-500 absolute top-2 left-0 right-0 flex justify-center pointer-events-none">
          <div className="bg-black/20 backdrop-blur-xl px-6 py-2 rounded-full flex items-center gap-6 pointer-events-auto border border-white/10 shadow-xl text-white">
            ...
            <button onClick={() => { navigator.clipboard.writeText(window.location.href); showMessage('連結已複製'); }} className="hover:text-white/60 transition-colors cursor-pointer text-white"><Share2 size={14} color="white"/></button>
      ...
      <motion.nav animate={{ y: showHeader ? 0 : -100 }} className="fixed top-0 left-0 w-full p-8 flex justify-between items-center z-[110] mix-blend-difference font-black tracking-tighter uppercase italic text-white">
        <div onClick={() => { setView({type:'home'}); setActiveMenu('All'); setActiveSubPath('All'); window.scrollTo(0,0); }} className="text-xl cursor-pointer hover:opacity-50 transition-all text-white">Curtis Chen</div>
      ...
      <AnimatePresence>
        {isSearchOpen && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="fixed top-24 left-0 w-full px-8 md:px-24 z-[105] text-white">
      ...
      <AnimatePresence mode="wait">
        ...
        {view.type === 'home' && (
          <motion.main key={activeMenu} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pt-40 px-8 md:px-24 pb-40 text-white font-sans text-white text-white">
            <div className="flex flex-row items-start gap-12 mb-32 text-white text-white">
              <motion.div initial={{ x: -40, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={transition} className="flex gap-4 text-white">
                <div className="text-7xl md:text-[9.5vw] font-black leading-none uppercase tracking-tighter font-serif vertical-text border-r border-white/10 pr-6 text-white">{activeMenu === 'All' ? '博物館' : activeMenu}</div>
                <div className="text-7xl md:text-[9.5vw] font-black leading-none uppercase tracking-tighter font-serif vertical-text pt-32 italic text-white">{activeMenu === 'All' ? '部落格' : '展覽室'}</div>
              </motion.div>
            ...
            <div className="space-y-8 mb-24 sticky top-0 bg-[#368C84]/90 backdrop-blur-lg z-[100] py-6 border-b border-white/5 text-white">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 text-white text-white">
                    ...
                    {activeMenu === 'All' ? (
                       ['All', ...featuredMenus].map(m => (
                        <div key={m} className="relative group flex items-center text-white">
                          ...
                          {isAdmin && m !== 'All' && (
                             <button onClick={() => toggleFMenu(m)} className="absolute -top-2 -right-2 p-1 bg-[#368C84] border border-white/20 rounded-full opacity-0 group-hover:opacity-100 transition-opacity text-white">
                ...
                <div className="flex items-center gap-4 text-[9px] uppercase tracking-[0.2em] font-black text-white/40 self-end md:self-auto text-white">
                   ...
                   <select className="bg-transparent border-none outline-none text-white transition-all font-black text-[10px] text-white" value={sortMode} onChange={(e) => setSortMode(e.target.value)}>
            ...
            <div className="grid grid-cols-1 text-white text-white">
              {activeMenu !== 'All' ? (
                        ...
                        {arts.map((art) => (
                          <motion.div key={art.id} onClick={() => setView({ type: 'article', id: art.id })} className="group py-12 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center cursor-pointer hover:bg-white/[0.02] px-8 -mx-8 relative text-white">
                             <div className="flex-1 text-white text-white">
                                <div className="flex items-center gap-3 text-[10px] uppercase tracking-widest text-white/40 italic font-bold mb-3 text-white">
                                  ...
                                  {art.status === 'draft' && <span className="bg-yellow-500/20 text-yellow-500 px-2 rounded-full text-[8px] tracking-widest border border-yellow-500/20 text-white">DRAFT</span>}
                                </div>
                ...
                processedArticles.map((art) => (
                  <motion.div key={art.id} onClick={() => setView({ type: 'article', id: art.id })} className="group py-16 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center cursor-pointer hover:bg-white/[0.02] transition-all px-8 -mx-8 relative text-white text-white">
                    <div className="flex-1 space-y-4 text-white text-white text-white">
                      ...
                      <h3 className="text-4xl md:text-7xl font-serif leading-tight group-hover:font-black group-hover:italic text-white text-white text-white">{art.title}</h3>
                      <p className="text-[9px] uppercase tracking-widest text-white/20 font-mono text-white/20 text-white">{new Date(art.publishDate.toDate()).toLocaleDateString()}</p>
                    </div>
                    {isAdmin && <button onClick={(e) => { e.stopPropagation(); setEditingArticle(art); setView({type:'cms'}); }} className="p-3 rounded-full border border-white/10 hover:bg-white hover:text-black transition-all cursor-pointer text-white text-white"><Edit3 size={16} color="white"/></button>}
                  </motion.div>
                ))
              )}
            </div>
        ...
        {view.type === 'article' && (
          <motion.article key="article" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={transition} className="pt-40 px-8 md:px-24 max-w-6xl mx-auto pb-60 text-white font-sans text-white text-white">
            <div className="flex justify-between items-center mb-16 border-b border-white/5 pb-8 text-white text-white text-white text-white">
              <div className="flex items-center gap-4 text-[10px] uppercase tracking-widest text-white/20 font-serif italic text-white text-white text-white text-white">
                ...
                {articles.find(a => a.id === view.id)?.subPath?.map((p, i) => (<React.Fragment key={i}><ChevronRight size={10} color="white" /><span className="text-white/40 text-white text-white">{p}</span></React.Fragment>))}
              </div>
              <div className="flex items-center gap-6 text-white text-white text-white">
                ...
                {isAdmin && <button onClick={() => { setEditingArticle(articles.find(a => a.id === view.id)); setView({type:'cms'}); }} className="flex items-center gap-3 text-[10px] uppercase tracking-widest text-white/40 border border-white/10 px-6 py-2 rounded-full font-black shadow-xl transition-all hover:bg-white hover:text-black uppercase text-white text-white text-white">編輯館藏</button>}
              </div>
            </div>
            {articles.find(a => a.id === view.id) && (
              <>
                <h1 className="text-5xl md:text-[7.5vw] font-black font-serif mb-24 leading-[1.05] tracking-tighter italic text-white text-white text-white">{articles.find(a => a.id === view.id).title}</h1>
                <div className="text-xl md:text-2xl leading-relaxed text-white/70 space-y-12 font-serif max-w-3xl mb-60 article-content text-white text-white text-white" dangerouslySetInnerHTML={{ __html: renderMarkdown(articles.find(a => a.id === view.id).content) }} />
                
                {/* 反應區 (修復互動) */}
                <div className="flex flex-col items-center gap-8 mb-40 border-y border-white/5 py-12 text-white text-white text-white text-white">
                ...
                {/* 留言區 (修復互動) */}
                <div className="max-w-3xl mx-auto mb-60 space-y-16 text-white text-white text-white text-white text-white">
                  <h3 className="text-2xl font-serif italic flex items-center gap-4 text-white text-white text-white">檔案評論區 <span className="text-xs font-mono opacity-20 tracking-tighter text-white text-white">({comments.length})</span></h3>
                  <div className="space-y-12 text-white text-white text-white text-white">
                    {comments.map(c => (
                      <div key={c.id} className="group flex gap-6 items-start text-white text-white text-white">
                        <div className="w-12 h-12 rounded-full bg-white/5 border border-white/10 flex-shrink-0 overflow-hidden shadow-2xl text-white text-white text-white">
                          {c.userPhoto ? <img src={c.userPhoto} alt="" className="w-full h-full object-cover text-white text-white" /> : <div className="w-full h-full flex items-center justify-center text-white/10 italic font-black font-serif uppercase text-white text-white text-white">?</div>}
                        </div>
                        <div className="flex-1 space-y-2 text-white text-white text-white text-white">
                          <div className="flex items-center justify-between text-white text-white text-white text-white">
                            <div className="flex items-center gap-3 text-white text-white text-white">
                              ...
                              <span className="text-[9px] text-white/20 font-mono italic text-white text-white text-white">{new Date(c.createdAt.toDate()).toLocaleDateString()}</span>
                            </div>
                          ...
                          <p className="text-white/60 font-serif text-lg leading-relaxed text-white text-white text-white text-white">${c.text}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  <div className="relative pt-12 text-white text-white text-white text-white text-white">
                    <textarea rows={4} className="w-full bg-white/5 border border-white/10 rounded-[50px] p-10 outline-none focus:border-[#368C84]/50 transition-all font-serif text-white placeholder-white/5 shadow-inner text-white text-white text-white text-white" value={newComment} onChange={e => setNewComment(e.target.value)} placeholder={user ? "撰寫評論..." : "請先登入以發表評論"} />
                    <div className="flex justify-end mt-4 text-white text-white text-white text-white">
                ...
                <div className="pt-32 border-t border-white/10 flex flex-col items-center text-white text-white text-white">
                    ...
                    <div className="w-full flex flex-row items-center justify-between gap-4 md:gap-12 px-2 text-white text-white text-white text-white">
                        <div onClick={() => { setView({type:'home'}); window.scrollTo(0,0); }} className="flex items-center gap-6 group cursor-pointer text-white flex-shrink-0 text-white text-white text-white">
                            <div className="p-4 md:p-6 rounded-full border border-white/20 group-hover:bg-white group-hover:text-black transition-all duration-700 shadow-2xl flex items-center justify-center text-white text-white text-white">
                                <ArrowLeft size={20} color="white" className="group-hover:stroke-black text-white text-white text-white text-white" />
                            </div>
                            <span className="text-[9px] md:text-[11px] uppercase tracking-[0.5em] text-white/30 group-hover:text-white whitespace-nowrap font-black hidden lg:inline text-white text-white text-white">Back to List</span>
                        </div>
                        {articles.filter(a => a.id !== view.id)[0] && (
                           <motion.div whileHover={{ scale: 1.05 }} onClick={() => { setView({type:'article', id: articles.filter(a => a.id !== view.id)[0].id}); window.scrollTo(0,0); }} className="flex flex-col items-center group cursor-pointer max-w-[50%] md:max-w-xl text-center text-white overflow-hidden text-white text-white text-white">
                              <h4 className="text-xl md:text-4xl font-serif text-white transition-all leading-tight tracking-tighter group-hover:italic group-hover:font-black truncate w-full text-white text-white text-white">
                        ...
                        <div onClick={() => { setView({type:'home'}); setActiveMenu('All'); window.scrollTo(0,0); }} className="flex items-center gap-6 group cursor-pointer text-white flex-shrink-0 text-white text-white text-white">
                            <span className="text-[9px] md:text-[11px] uppercase tracking-[0.4em] text-white/30 group-hover:text-white whitespace-nowrap font-black hidden lg:inline text-white text-white text-white text-white">Return Home</span>
                            <div className="p-3 md:p-5 rounded-full border border-white/20 group-hover:bg-white group-hover:text-black transition-all duration-700 shadow-2xl flex items-center justify-center text-white text-white text-white">
      ...
      <footer className="p-12 md:p-24 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 text-[8px] text-white/10 uppercase tracking-[0.6em] font-light italic text-white/10 text-white text-white text-white text-white text-white">
        <SocialIconsList className="text-white text-white text-white text-white" />
        <InteractiveLink className="font-black text-white/20 text-white text-white text-white text-white text-white" onClick={() => setView({type:'home'})}>© 2026 CURTIS CHEN — MUSEUM BLOG ARCHIVE</InteractiveLink>
        <div className="flex items-center gap-8 transition-all hover:text-white/40 font-black uppercase text-white/20 text-white text-white text-white"><ShieldCheck size={10} color="white"/> Private Encryption Active</div>
      </footer>
    </div>
  );
}