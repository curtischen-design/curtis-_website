...
/**
 * Markdown 渲染器：完美支援 Obsidian Callouts (修正加強版)
 */
const renderMarkdown = (text) => {
  if (!text) return '';
  let processed = text;

  // 1. Obsidian Callout 渲染
  ...
  processed = processed.replace(calloutRegex, (match, type, title, content) => {
    ...
    if (type.toLowerCase() === 'quote') {
      ...
      return `<div class="my-10 p-8 bg-[#368C84]/20 border-l-4 border-[#368C84] rounded-r-3xl shadow-xl text-white">
                <div class="flex items-center gap-3 mb-4 opacity-50 font-black text-white"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg><span class="uppercase tracking-[0.4em] text-[10px]">Archives Information</span></div>
...
const CMSDashboard = ({ user, setView, editData, showMessage, menuList, featuredMenus, baseMenuItems, menuQuotes }) => {
  ...
  const updateMenuQuote = async (name, quote) => {
    const updated = { ...menuQuotes, [name]: quote };
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="pt-32 px-8 md:px-24 max-w-4xl mx-auto pb-40 text-white font-sans text-white">
      ...
      <div className="space-y-16 text-white text-white">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 text-white">
          <div className="space-y-4 text-white">
            ...
            <div className="flex flex-wrap gap-2 text-white text-white">
              {menuList.map(m => (
          ...
          <div className="space-y-4 text-white">
            <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold ml-1 flex items-center gap-2 text-white text-white"><Calendar size={12}/> 2. 檔案時間</label>
        ...
        <div className="space-y-4 text-white">
          <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold ml-1 text-white">4. 子路徑 (例如: 大一 / 生活)</label>
      ...
      <div className="mt-40 space-y-12 border-t border-white/10 pt-40 text-white">
        <div className="p-10 bg-white/5 rounded-[60px] border border-white/5 space-y-8 text-white">
          <h3 className="text-[10px] uppercase tracking-[0.4em] text-white/30 flex items-center gap-3 text-white"><Star size={14}/> 選單策展設定 (常駐與名言)</h3>
          <div className="grid grid-cols-1 gap-8 text-white">
            {menuList.map(name => (
              <div key={name} className="flex flex-col md:flex-row md:items-center gap-6 bg-white/[0.02] p-6 rounded-3xl border border-white/5 text-white">
                <button onClick={() => updateFMenus(name)}
                  className={`px-6 py-2 rounded-full text-[10px] tracking-widest border transition-all ${featuredMenus.includes(name) ? 'bg-white text-black border-white font-black' : 'border-white/10 text-white/30'}`}
                >
                ...
                <div className="flex-1 text-white">
                   ...
                   <input className="w-full bg-transparent border-b border-white/10 py-2 outline-none text-white italic" value={menuQuotes[name] || ''} onChange={(e) => updateMenuQuote(name, e.target.value)} placeholder="輸入展廳名言..." />
                </div>
              </div>
            ))}
          </div>
...
export default function App() {
  ...
  const processedArticles = useMemo(() => {
    let list = articles.filter(art => {
      ...
      return (isAdmin || isPublic) && mMenu && mSearch;
    });
    ...
    return list;
  }, [articles, activeMenu, searchQuery, isAdmin, view.type]);
  ...
  const groupedArticles = useMemo(() => {
    ...
    processedArticles.forEach(art => {
       const groupName = art.subPath?.[0] || 'Unsorted';
    ...
    <div className="bg-[#368C84] text-white min-h-screen font-sans selection:bg-white selection:text-[#368C84] text-white overflow-x-hidden">
              ...
              <div className="flex flex-col items-center gap-12 md:gap-20 w-full text-white text-white">
                {menuRows.map((row, rid) => (
                  <div key={rid} className="flex flex-wrap justify-center gap-12 md:gap-24 w-full px-4 text-white">
                    {row.map((item) => (
                      <InteractiveLink key={item} onClick={() => { setActiveMenu(item); setView({type:'home'}); setIsMenuOpen(false); window.scrollTo(0,0); }} className="text-3xl md:text-5xl font-serif font-light tracking-tight px-2 text-white">
                        {item}
                      </InteractiveLink>
                    ))}
                  </div>
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.main key={activeMenu} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pt-40 px-8 md:px-24 pb-40 text-white font-sans text-white text-white">
            <div className="flex flex-row items-start gap-12 mb-32 text-white">
              ...
              <div className="flex-1 space-y-6 pt-16 hidden md:block text-white/60 text-white text-white text-white">
                <p className="text-xs uppercase tracking-[0.4em] text-white/40 border-b border-white/10 pb-6 font-black text-white text-white">
                  {activeMenu === 'All' ? 'Digital Curations by Curtis Chen' : `Gallery: ${activeMenu}`}
                </p>
                <p className="text-lg font-serif italic text-white/60 leading-relaxed max-w-sm text-white text-white">
                   {activeMenu === 'All' ? '致力於收藏生活中的美學動態，將瑣碎的感知轉化為永恆的數位檔案。' : (menuQuotes[activeMenu] || '此展廳正在進行數位修復中。')}
                </p>
                <SocialIconsList className="!gap-4 !text-white/10 pt-4 text-white text-white" />
              </div>
            </div>
                  ...
                  <div className="flex flex-col text-white">
                    <div className="flex items-center gap-4 overflow-x-auto scrollbar-hide font-black text-white">
                      ...
                      {['All', ...featuredMenus].map(m => (
                        <button key={m} onClick={() => { setActiveMenu(m); setActivePath([]); }} className={`px-6 py-2 rounded-full text-[10px] border whitespace-nowrap cursor-pointer relative group ${activeMenu === m ? 'bg-white text-black border-white shadow-xl' : 'border-white/10 text-white/40 hover:border-white/60'}`}>{m}</button>
                      ))}
                    </div>
                    <span className="text-[9px] text-white/10 ml-16 mt-2 tracking-widest font-black text-white text-white">[（看更多請點右上選單）]</span>
                  </div>
              ...
              {activeMenu !== 'All' ? (
                Object.entries(groupedArticles).map(([group, arts]) => (
                  <div key={group} className="mb-24">
                     <h2 className="text-5xl md:text-8xl font-serif italic text-white/10 mb-12 border-b border-white/5 pb-8">{group}</h2>
                        ...
                        {arts.map((art, idx) => (
                          <motion.div key={art.id} onClick={() => setView({ type: 'article', id: art.id })} className="group py-12 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center cursor-pointer hover:bg-white/[0.02] px-8 -mx-8 relative text-white">
                             <div className="flex-1 text-white">
                                ...
                                <p className="text-[9px] uppercase tracking-widest text-white/20 font-mono mt-4 text-white">{new Date(art.publishDate.toDate()).toLocaleDateString()}</p>
                             </div>
                             <ArrowUpRight size={24} className="mt-8 md:mt-0 text-white/20 group-hover:text-white transition-all" />
                          </motion.div>
                        ))}
                     </div>
                  </div>
                  ...
                  <motion.div key={art.id} onClick={() => setView({ type: 'article', id: art.id })} className="group py-16 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center cursor-pointer hover:bg-white/[0.02] transition-all px-8 -mx-8 relative text-white text-white">
                    <div className="flex-1 space-y-4 text-white">
                      ...
                      <div className="flex gap-6 text-[9px] uppercase tracking-widest text-white/20 font-mono text-white/20">
                        ...
                        <span className="flex items-center gap-1 text-white"><MessageSquare size={10}/> {art.commentsCount || 0}</span>
                      </div>
                    </div>
              ...
              <>
                ...
                <div className="max-w-3xl mx-auto mb-60 space-y-16 text-white text-white">
                    ...
                    {comments.map(c => (
                      <div key={c.id} className="group flex gap-6 items-start text-white">
                        <div className="w-12 h-12 rounded-full bg-white/5 border border-white/10 flex-shrink-0 overflow-hidden shadow-2xl text-white text-white">
                          {c.userPhoto ? <img src={c.userPhoto} alt="" className="w-full h-full object-cover text-white" /> : <div className="w-full h-full flex items-center justify-center text-white/10 italic font-black font-serif uppercase">?</div>}
                        </div>
                        <div className="flex-1 space-y-2 text-white">
                          <div className="flex items-center justify-between text-white text-white">
                            ...
                            <div className="flex gap-4 opacity-0 group-hover:opacity-100 transition-opacity text-white">
                  ...
                  <div className="relative pt-12 text-white text-white text-white">
                    ...
                    {user ? (
                      <button onClick={postComment} className="absolute bottom-8 right-10 bg-white text-[#368C84] px-10 py-4 rounded-full font-black text-xs flex items-center gap-4 cursor-pointer shadow-2xl hover:scale-105 transition-all text-[#368C84]">Submit <Send size={18} color="#368C84"/></button>
                    ) : (
                      <button onClick={() => signInWithPopup(auth, provider)} className="absolute bottom-8 right-10 bg-white/10 text-white px-10 py-4 rounded-full font-black text-xs cursor-pointer hover:bg-white/20 transition-all text-white text-white">Sign In to Post</button>
                    )}
                  </div>
                </div>

                <div className="pt-32 border-t border-white/10 flex flex-col items-center text-white">
                    ...
                    <div className="w-full flex flex-row items-center justify-between gap-4 md:gap-12 px-2 text-white">
                        <div onClick={() => { setView({type:'home'}); window.scrollTo(0,0); }} className="flex items-center gap-6 group cursor-pointer text-white flex-shrink-0 text-white text-white">
                            ...
                            <span className="text-[9px] md:text-[11px] uppercase tracking-[0.5em] text-white/30 group-hover:text-white whitespace-nowrap font-black hidden lg:inline text-white text-white">Back to List</span>
                        </div>
        ...
        {view.type === 'cms' && isAdmin && (
          <CMSDashboard user={user} setView={setView} editData={editingArticle} showMessage={showMessage} menuList={dynamicMenuItems} socialLinks={socialLinks} baseMenuItems={baseMenuItems} featuredMenus={featuredMenus} menuQuotes={menuQuotes} />
        )}
      </AnimatePresence>
