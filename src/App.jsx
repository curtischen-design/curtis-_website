...
/**
 * Markdown 渲染器：支援 H1-H6 與修復後的 Obsidian Callouts
 */
const renderMarkdown = (text) => {
  if (!text) return '';
  return text
    // 修復：Obsidian Callouts 正則表達式優化，支援多行內容捕捉
    .replace(/^> \[!quote\](.*?)\n([\s\S]*?)(?=\n\n|\n[^\s>#]|$)/gim, (match, title, content) => {
      return `<div class="my-10 p-8 bg-white/5 border-l-4 border-white/40 rounded-r-3xl italic font-serif text-white/90 shadow-inner">“${content.replace(/^>\s?/gm, '').trim()}”</div>`;
    })
    .replace(/^> \[!info\](.*?)\n([\s\S]*?)(?=\n\n|\n[^\s>#]|$)/gim, (match, title, content) => {
      return `<div class="my-10 p-8 bg-[#368C84]/20 border-l-4 border-[#368C84] rounded-r-3xl text-white/80 shadow-xl"><strong class="block mb-4 uppercase tracking-[0.3em] text-[10px] opacity-40 font-black">Information Notice</strong>${content.replace(/^>\s?/gm, '').trim()}</div>`;
    })
    // 標題渲染
...
const CMSDashboard = ({ user, setView, editData, showMessage, menuList, socialLinks, baseMenuItems, featuredMenus }) => {
  ...
  const [fMenus, setFMenus] = useState(featuredMenus || []);
  ...
  const toggleFMenu = (name) => {
    const updated = fMenus.includes(name) ? fMenus.filter(m => m !== name) : [...fMenus, name];
    setFMenus(updated);
    updateSettings('fMenus', { items: updated });
  };
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="pt-32 px-8 md:px-24 max-w-4xl mx-auto pb-40 text-white font-sans">
      ...
      <div className="space-y-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 text-white">
          ...
          <div className="space-y-4">
            <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold ml-1 flex items-center gap-2 text-white"><Calendar size={12}/> 2. 檔案時間</label>
        ...
        <div className="space-y-4 text-white">
          <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold ml-1 text-white">4. 內容 (支援 Obsidian Callouts)</label>
          <textarea required rows={12} className="w-full bg-black/10 border border-white/5 p-8 rounded-[40px] text-white/80 font-mono text-sm outline-none focus:border-[#368C84]/50 transition-all text-white" value={content} onChange={e => setContent(e.target.value)} placeholder="使用 > [!quote] 或 > [!info] 標註重點..." />
        </div>
      ...
      <div className="mt-40 space-y-12 border-t border-white/10 pt-40 text-white">
        ...
        <div className="p-10 bg-white/5 rounded-[60px] border border-white/5 space-y-8 text-white">
          ...
          <p className="text-[9px] text-white/20 italic">勾選以下選單名稱，使其始終顯示在首頁的「精選」橫列中。</p>
          <div className="flex flex-wrap gap-4 text-white">
            {menuList.map(name => (
              <button key={name} onClick={() => toggleFMenu(name)}
                className={`px-6 py-2 rounded-full text-[10px] tracking-widest border transition-all ${fMenus.includes(name) ? 'bg-[#368C84] border-[#368C84] font-black text-white' : 'border-white/10 text-white/30 hover:border-white/40'}`}
              >
                {name}
              </button>
            ))}
        ...
        <div className="p-10 bg-white/5 rounded-[60px] border border-white/5 space-y-8">
          <h3 className="text-[10px] uppercase tracking-[0.4em] text-white/30 flex items-center gap-3 text-white"><Settings2 size={14}/> 導覽路徑移除 (清理空白項目)</h3>
        ...
        <div className="p-10 bg-white/5 rounded-[60px] border border-white/5 space-y-10 shadow-2xl">
          <h3 className="text-[10px] uppercase tracking-[0.4em] text-white/30 flex items-center gap-3 text-white"><LinkIcon size={14}/> 全站社群連結設定</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-white">
            {Object.keys(links).map(key => (
              <div key={key} className="space-y-2">
                <label className="text-[9px] uppercase tracking-widest text-white/20 ml-4 font-bold">{key}</label>
                <input className="w-full bg-black/30 border border-white/5 rounded-full px-6 py-3 text-[11px] outline-none focus:border-[#368C84] text-white/60 transition-all" value={links[key] || ''} onChange={e => setLinks({...links, [key]: e.target.value})} placeholder={`Paste URL...`} />
              </div>
            ))}
          </div>
          <button onClick={() => updateSettings('links', links)} className="w-full py-5 bg-white/5 border border-white/10 rounded-full text-[10px] uppercase tracking-[0.4em] hover:bg-white hover:text-black transition-all font-black cursor-pointer">同步社群連結</button>
        </div>
      </div>
    </motion.div>
  );
...
export default function App() {
  ...
  const [featuredMenus, setFeaturedMenus] = useState([]); // 變更：常駐精選項
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (atBottom) { setMenuPulseKey(prev => prev + 1); } // 觸底呼吸動畫觸發
  ...
  const toggleBookmark = async (id) => {
    ...
    else { await setDoc(ref, { addedAt: Timestamp.now() }); showMessage('成功入藏'); }
  };
  ...
  const deleteComment = async (id) => {
    if (!confirm("確定將此檔案評論從歷史中刪除？")) return;
  ...
  return (
    <div className="bg-[#368C84] text-white min-h-screen font-sans selection:bg-white selection:text-[#368C84] text-white overflow-x-hidden">
          ...
          <motion.main key="home" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pt-40 px-8 md:px-24 pb-40 text-white">
            <div className="flex flex-row items-start gap-12 mb-32">
              ...
              <div className="flex-1 space-y-6 pt-16 hidden md:block text-white/60">
                <p className="text-xs uppercase tracking-[0.6em] text-white/40 border-b border-white/10 pb-6 font-black text-white/40 tracking-[0.4em]">Digital Curations by Curtis Chen</p>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex flex-col">
                  <div className="flex items-center gap-4 overflow-x-auto scrollbar-hide font-black text-white">
                    <span className="text-[11px] uppercase tracking-[0.4em] text-white flex-shrink-0 flex items-center gap-2 font-black">精選</span>
                    {/* 修改：顯示 All + 管理員設定的常駐選單項 */}
                  ...
                  <span className="text-[9px] text-white/10 ml-16 mt-2 tracking-widest font-black">[（看更多請點右上選單）]</span>
                </div>
                ...
                <div className="flex items-center gap-4 text-[9px] uppercase tracking-[0.2em] font-black text-white/40 self-end md:self-auto">
                   ...
                   <span>館藏排序</span>
                   <select 
                      className="bg-transparent border-none outline-none text-white cursor-pointer hover:text-white transition-all font-black text-[10px]"
                      value={sortMode}
                   ...
                   >
                      ...
                      <option value="comments" className="bg-[#368C84]">Hot Discussion</option>
                   </select>
                </div>
              </div>
            </div>
                ...
                <motion.div key={art.id} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.05 }} onClick={() => { setView({ type: 'article', id: art.id }); window.scrollTo(0,0); }} className="group py-16 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center cursor-pointer hover:bg-white/[0.02] transition-all px-8 -mx-8 relative text-white">
                  <div className="flex-1 space-y-4">
                    <div className="flex items-center gap-3 flex-wrap text-[10px] uppercase tracking-widest text-white/40 italic font-bold">
                      ...
                      {art.status === 'draft' && <span className="bg-yellow-500/20 text-yellow-500 px-2 rounded-full text-[8px] tracking-widest ml-2 border border-yellow-500/20">DRAFT</span>}
                    </div>
                    ...
                    <div className="flex gap-6 text-[9px] uppercase tracking-widest text-white/20 font-mono">
          ...
          <motion.article key="article" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={transition} className="pt-40 px-8 md:px-24 max-w-6xl mx-auto pb-60 text-white font-sans">
            ...
            {articles.find(a => a.id === view.id) && (
              <>
                ...
                {/* Comments Section */}
                <div className="max-w-3xl mx-auto mb-60 space-y-16 text-white">
                  <h3 className="text-2xl font-serif italic flex items-center gap-4 text-white text-white">檔案評論區 <span className="text-xs font-mono opacity-20 tracking-tighter">({comments.length})</span></h3>
                  <div className="space-y-12 text-white text-white">
                    {comments.map(c => (
                ...
                <div className="pt-32 border-t border-white/10 flex flex-col items-center">
                    ...
                    <div className="w-full flex flex-row items-center justify-between gap-4 md:gap-12 px-2 text-white">
                        <div onClick={() => { setView({type:'home'}); window.scrollTo(0,0); }} className="flex items-center gap-6 group cursor-pointer text-white flex-shrink-0 text-white">
                            ...
                            <span className="text-[9px] md:text-[12px] uppercase tracking-[0.5em] text-white/30 group-hover:text-white whitespace-nowrap font-black hidden lg:inline">Back to List</span>
                        </div>
                        {articles.filter(a => a.id !== view.id)[0] && (
                           <motion.div whileHover={{ scale: 1.05 }} onClick={() => { setView({type:'article', id: articles.filter(a => a.id !== view.id)[0].id}); window.scrollTo(0,0); }} className="flex flex-col items-center group cursor-pointer max-w-[50%] md:max-w-xl text-center text-white overflow-hidden">
                              <h4 className="text-xl md:text-4xl font-serif text-white transition-all leading-tight tracking-tighter group-hover:italic group-hover:font-black truncate w-full text-white">
                        ...
                        <div onClick={() => { setView({type:'home'}); setActiveMenu('All'); window.scrollTo(0,0); }} className="flex items-center gap-6 group cursor-pointer text-white flex-shrink-0 text-white">
                            <span className="text-[9px] md:text-[12px] uppercase tracking-[0.5em] text-white/30 group-hover:text-white whitespace-nowrap font-black hidden lg:inline">Return Home</span>
        ...
        {view.type === 'bookmarks' && (
          <motion.main key="bookmarks" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pt-40 px-8 md:px-24 pb-40 min-h-screen text-white">
             <div className="mb-24 border-b border-white/10 pb-12 text-white">
               ...
               <h1 className="text-6xl md:text-8xl font-serif italic font-light leading-none text-white text-white">My Private Gallery.</h1>
             </div>
             <div className="grid grid-cols-1">
               {articles.filter(a => userBookmarks.includes(a.id)).map((art, index) => (
                   <motion.div key={art.id} onClick={() => setView({type:'article', id: art.id})} className="group py-12 border-b border-white/5 flex justify-between items-center cursor-pointer hover:bg-white/[0.02] px-8 -mx-8 transition-all text-white">
                     <div><p className="text-[9px] uppercase tracking-widest text-white/40 mb-3 italic font-bold">{art.mainMenu} {art.subPath?.length > 0 && `/ ${art.subPath.join(' / ')}`}</p><h3 className="text-3xl md:text-5xl font-serif text-white transition-all group-hover:font-black group-hover:italic text-white">{art.title}</h3></div>
      ...
      <footer className="p-12 md:p-24 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 text-[8px] text-white/10 uppercase tracking-[0.6em] font-light italic text-white/10 text-white">
        <SocialIconsList />
